...
const RegistrationForm: React.FC = () => {
    ...
    if (validate()) {
      try {
        ...
        if (response.ok) {
          ...
          router.push('/reistration_page'); // Redirect or handle as needed
        } else {
          alert('Error: ' + result.error); // Show error message
        }
      } catch (error) {
        alert('Error: ' + error.message); // Show network error message
      }
    }
  };
  ...
  return (
    <form className={styles.formContainer} onSubmit={handleSubmit}>
      ...
      <button type="submit" className={styles.submitButton}onClick={()=>{router.push('/reistration_page')}}>
        Register
      </button>
    </form>
  );
